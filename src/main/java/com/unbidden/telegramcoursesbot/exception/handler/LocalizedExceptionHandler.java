...
public interface LocalizedExceptionHandler extends ExceptionHandler {
    SendMessage compileSendMessageFromLocalizedExc(@NonNull UserEntity user,
            @NonNull LocalizedException exc);

    @Override
    default SendMessage compileSendMessage(@NonNull UserEntity user, @NonNull Exception exc) {
        if (exc instanceof LocalizedException) {
            return compileSendMessageFromLocalizedExc(user, (LocalizedException)exc);
        }
