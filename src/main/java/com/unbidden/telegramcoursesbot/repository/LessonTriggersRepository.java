...
public interface LessonTriggersRepository extends JpaRepository<LessonTrigger, Long> {
    @NonNull
    @Query("from LessonTrigger lt left join fetch lt.user u left join fetch lt.progress p "
            + "left join fetch p.course c where lt.target > current_timestamp")
    List<LessonTrigger> findAllExpired();
    ...
    @NonNull
    @Query("from LessonTrigger lt left join fetch lt.user u left join fetch lt.progress p "
            + "left join fetch p.course c where u.id = :userId and "
