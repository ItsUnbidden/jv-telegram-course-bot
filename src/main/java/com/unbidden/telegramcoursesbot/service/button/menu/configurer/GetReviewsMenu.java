...
import com.unbidden.telegramcoursesbot.service.button.handler.GetArchiveReviewsButtonHandler;
import com.unbidden.telegramcoursesbot.service.button.handler.GetNewReviewsButtonHandler;
...
public class GetReviewsMenu implements MenuConfigurer {
    ...
    public void configure() {
        ...
        firstPage.setButtonsFunction((u, p) -> {
            ...
            buttons.addAll(courseService.getAll().stream().map(c -> (Button)new TransitoryButton(
                    localizationLoader.getLocalizationForUser(COURSE_NAME.formatted(
                    c.getName()), u).getData(), c.getId().toString(), 1)).toList());
        ...
        secondPage.setButtonsFunction((u, p) -> List.of(new TerminalButton(localizationLoader
                ...
                GET_ARCHIVE_REVIEWS, getArchiveReviewsHandler)));
