...
public class ArchiveReviewsDaoImpl implements ArchiveReviewsDao {
    ...
    @PostConstruct
    public void init() {
        tempPath = Path.of(tempDirectoryPath);
        try {
            Files.createDirectories(tempPath);
        } catch (IOException e) {
            throw new ArchiveReviewsException("Unable to create a new directory in "
    ...
    public Path createTempFile(@NonNull String name) {
        try {
            ...
            throw new ArchiveReviewsException("Unable to create a new temp file in directory "
    ...
    public Path write(@NonNull Path path, @NonNull String content) {
        try {
            ...
            throw new ArchiveReviewsException("Unable to append to the temp file by path "
                    + path, e);
        }
    }
    ...
    public InputStream read(@NonNull Path path) {
        try {
            ...
            throw new ArchiveReviewsException("Unable to read the temp file by path "
                    + path, e);
        }
    }
    ...
    public void delete(@NonNull Path path) {
        try {
            ...
            throw new ArchiveReviewsException("Unable to delete the temp file by path "
                    + path, e);
        }
    }
}
