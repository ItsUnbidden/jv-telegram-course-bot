...
public class RefundButtonHandler extends AbstractButtonHandler {
    ...
    public void handle(UserEntity user, Bot bot, Map<String, String> params) {
        ...
        final String confirmationPhrase = localizationLoader.localize(Localizations.Service.REFUND_CONFIRMATION_PHRASE, user,
        ...
        sessionService.createSession(user, bot, p -> {
            paymentService.refund(p.user(), p.bot(), courseId, confirmationPhrase, p.messages());
        });

        clientManager.getClient(bot).sendMessage(user, localizationLoader.localize(
                Localizations.Service.REFUND_CONFIRMATION_REQUEST, user,
                new Localizations.Service.RefundConfirmationRequestParams(confirmationPhrase)));
    }
}
