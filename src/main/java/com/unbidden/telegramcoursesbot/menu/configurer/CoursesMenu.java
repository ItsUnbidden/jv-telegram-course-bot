...
public class CoursesMenu implements MenuConfigurer {
    ...
    public Menu configure() {
        ...
        firstPage.setLocalizationFunction(p -> loader.localize(Localizations.Menu.MY_COURSES_PAGE_0, p.user()));
        firstPage.setButtonsFunction(p -> List.of(
            new TransitoryButton(loader.localize(Localizations.Button.AVAILABLE_COURSES, p.user()).getData(), 1),
            new TransitoryButton(loader.localize(Localizations.Button.MY_COURSES, p.user()).getData(), 2)
        ));
        ...
        secondPage.setLocalizationFunction(p -> loader.localize(Localizations.Menu.MY_COURSES_PAGE_1, p.user()));
        secondPage.setButtonsFunction(p -> {
            final List<CourseResponseDto> availableCourses = courseService.getAllAvailableByUser(p.user(), p.bot());
        ...
        thirdPage.setLocalizationFunction(p -> loader.localize(Localizations.Menu.MY_COURSES_PAGE_2, p.user()));
        thirdPage.setButtonsFunction(p -> {
            final List<CourseMenuDto> dtos = courseService.getCourseMenuDtosForOwnedCourses(p.user(), p.bot());
            final List<Button> buttons = new ArrayList<>();
        ...
        fourthPage.setLocalizationFunction(p -> loader.localize(Localizations.Menu.MY_COURSES_PAGE_3, p.user()));
        fourthPage.setButtonsFunction(p -> {
            final List<Button> buttons = new ArrayList<>();
            final Long courseId = Long.parseLong(p.params().get(COURSE_ID_PARAM));

            buttons.add(new TerminalButton(loader.localize(Localizations.Button.BEGIN_COURSE, p.user()).getData(), courseId.toString(), initiateCourseHandler));

            final CourseMenuDto dto = courseService.getCourseMenuDtoForCourse(p.user(), p.bot(), courseId);

            if (dto.isCompleted()) {
                buttons.add(new TransitoryButton(loader.localize(Localizations.Button.SELECT_COURSE_STAGE, p.user()).getData(), 6));

                if (dto.isAdvancedReviewPresent() || dto.isBasicReviewPresent()) {
                    buttons.add(new TransitoryButton(loader.localize(Localizations.Button.UPDATE_REVIEW_OPTIONS, p.user()).getData(),
                            IS_ADVANCED_REVIEW_PRESENT_PARAM, String.valueOf(dto.isAdvancedReviewPresent()), 4));
                } else {
                    buttons.add(new TerminalButton(loader.localize(Localizations.Button.LEAVE_REVIEW, p.user()).getData(), sendBasicReviewHandler));
                }
            }
            if (dto.isRefundable()) {
                buttons.add(new TerminalButton(loader.localize(Localizations.Button.REFUND, p.user()).getData(), refundHandler));
            }
            buttons.add(new BackwardButton(loader.localize(Localizations.Button.BACK, p.user()).getData()));
        ...
        fifthPage.setLocalizationFunction(p -> loader.localize(Localizations.Menu.MY_COURSES_PAGE_4, p.user()));
        ...
        sixthPage.setLocalizationFunction(p -> loader.localize(Localizations.Menu.MY_COURSES_PAGE_5, p.user()));
        ...
        seventhPage.setLocalizationFunction(p -> loader.localize(Localizations.Menu.MY_COURSES_PAGE_6, p.user()));
