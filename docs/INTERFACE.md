# Interface
Here I'll explain the **interface** of the platform. Generally, interaction with bots is conducted through a combination of **commands** and **menus**.
- **Commands** are needed to call a *specific menu*.
- User navigates through the **menu** to access *features*.

Available **commands** can be found using the `Menu` button. Only commands that the user can actually access are visible. For example, a user without any special role will not see the `/admin` command.
...
`/start` — shows initial *start message* (works the same as `/creator`) and registers the user on the platform.
...
`/support` — opens the support menu. Allows the user to request support from the *Director*, *Creator*, or *Support* roles. I explain the support feature in more detail [here](/docs/FEATURES.md/#support).
...
- **Get mapping** — This is a useful function since it allows setting up differently localized versions of content.
  - Send mapping ID.
    ...
    - **Remove localization**
      - Send the language code of the localization you want to remove.
...
- Select a course
  ...
  - **Toggle maintenance** — enables/disables maintenance for this course. If enabled, no user can interact with this course. 
  - **Refund stage** — specifies a lesson after which a refund is impossible.
    - Send lesson index. A negative number means a refund is not supported.
    ...
    - Select a lesson
      ...
      - One of two options will be available depending on whether you have homework for this lesson:
        ...
        - **Homework settings**
          ...
          - **Media types** — defines what media types are allowed as a response to this homework.
  ...
  - **Delete course** — deletes this course. You have to confirm this by sending the course's internal name. The initial course cannot be deleted.
    - Send the course's internal name.

`/localization` — this is a **debug command** available only to the *Director*. It allows loading a **specific localization**. You need to include two parameters:
- Localization name
- Language code

Be aware that this particular command is **unique** in how the *parameters* are supplied. You need to enter this command **like this**: `/localization localization_name language_code`. It's unlikely you will need it, unless you want to see how particular localization looks in Telegram.

`/post` — opens the **post** menu. Here, you can send a message to a certain group of users in this bot. For example, you can send a message to all *Mentors*. Be aware that this feature is quite heavy on performance, and only one `Post` operation can be conducted at a time due to this issue.
- **List of roles** — choose a role, users of which will receive your message.
  - Send your message.
- **Custom role set** — define a custom role set.
  - **Send the role set** — role names must be separated by `whitespace`.
    - Send the message
...
- Select course — choose a **specific course** or **all courses**.
  - **New reviews** — shows three of the newest reviews to you. You have two or three options for each:
    ...
    - Answer/Update comment — allows sending content for a new comment. Editing is possible only if you are the one who sent it in the first place.
      - Send content.

  - **Archive reviews** — compiles a text file with all available reviews for this course (or all courses). Content is not included in this file and can be viewed with the `/content` command using IDs.

`/statistics` — opens the **statistics menu**. This menu allows requesting some statistical data on the bot's performance. More on statistics you can read *[here](/docs/FEATURES.md/#statistics)*.
...
`/generalban` — opens the **general ban** menu. This type of ban prevents the user from interacting with the platform **completely**. Works largely the same as a bot-wide ban. Check *[Creator](#creator)* if you want more info.
...
`/generalpost` — opens a **general post** menu. Here, you can send a message to a certain group of users across all bots. For example, you can send a message to all creators. Works largely the same as its bot-wide equivalent. Check *[Creator](#creator)* if you want more info.
